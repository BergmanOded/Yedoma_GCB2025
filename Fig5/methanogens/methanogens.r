...
library(viridis)
library(pheatmap)
otu <- read.table(file = "/home/oded/Documents/orit_sivan/articles/oded/NGS/heatmap/methanogens/feature_table.tsv", sep = ",", header = T, 
                  comment.char = "", check.names = FALSE)

taxonomy <- read.table(file = "/home/oded/Documents/orit_sivan/articles/oded/NGS/heatmap/methanogens/taxonomy.tsv", sep = ",", header = T, 
                  comment.char = "", check.names = FALSE)
...
metadata <- read.table(file = "/home/oded/Documents/orit_sivan/articles/oded/NGS/heatmap/methanogens/sample-metadata.tsv", sep = "\t", header = T, 
...
hm_methnogens <- phyloseq(OTU, SAMPLE, taxasums)

# Extract the abundance matrix from the phyloseq object
otu_matrix <- as.matrix(otu_table(hm_methnogens))




# Optional: Prepare a taxonomy annotation dataframe for the rows (OTUs)
taxa_annotation <- as.data.frame(tax_table(hm_methnogens))
...
heatmap_arch <-  pheatmap(otu_matrix,
        ...
        pdf("/home/oded/Documents/orit_sivan/articles/oded/NGS/heatmap/methanogens/24.11.18_heatmap_methanogen_yedome.pdf",  width = 10, height = 4, family="ArialMT")
        heatmap_arch
        dev.off()
